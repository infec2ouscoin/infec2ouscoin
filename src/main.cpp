...
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // infec2ouscoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "infec2ouscoin Signed Message:\n";
...
{
    // infec2ouscoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
static const int64 nTargetTimespan = 4 * 60 * 60; // infec2ousCoin: every 4 hours
static const int64 nTargetSpacing = 60; // infec2ousCoin: 1 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // infec2ouscoin: This fixes an issue where a 51% attack can change difficulty at will.
...
{
    // infec2ouscoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("infec2ouscoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("infec2ouscoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static infec2ouscoinMiner(CWallet *pwallet)
{
    printf("infec2ouscoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("infec2ouscoin-miner");
    ...
    try { loop {
        ...
        printf("Running infec2ouscoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("infec2ouscoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&infec2ouscoinMiner, pwallet));
}
