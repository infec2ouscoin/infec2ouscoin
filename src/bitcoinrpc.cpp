...
Value stop(const Array& params, bool fHelp)
{
    ...
    if (fHelp || params.size() > 1)
        throw runtime_error(
            "stop\n"
            "Stop infec2ouscoin server.");
    // Shutdown will take long enough that the response should get back
    StartShutdown();
    return "infec2ouscoin server stopping";
}
...
string HTTPPost(const string& strMsg, const map<string,string>& mapRequestHeaders)
{
    ostringstream s;
    s << "POST / HTTP/1.1\r\n"
      << "User-Agent: infec2ouscoin-json-rpc/" << FormatFullVersion() << "\r\n"
      << "Host: 127.0.0.1\r\n"
...
static string HTTPReply(int nStatus, const string& strMsg, bool keepalive)
{
    if (nStatus == HTTP_UNAUTHORIZED)
        return strprintf("HTTP/1.0 401 Authorization Required\r\n"
            "Date: %s\r\n"
            "Server: infec2ouscoin-json-rpc/%s\r\n"
...
void StartRPCThreads()
{
        ...
        || weakPassword(mapArgs["-rpcpassword"]))
    {
        ...
        string strWhatAmI = "To use infec2ouscoind";
        ...
        uiInterface.ThreadSafeMessageBox(strprintf(
            _("%s, you must set a secure rpcpassword in the configuration file:\n"
              ...
              "rpcuser=infec2ouscoinrpc\n"
              "rpcpassword=%s\n"
              "(you do not need to remember this password)\n"
              "The username and password MUST NOT be the same.\n"
              "If the file does not exist, create it with owner-readable-only file permissions.\n"
              "It is also recommended to set alertnotify so you are notified of problems;\n"
              "for example: alertnotify=echo %%s | mail -s \"infec2ouscoin Alert\" admin@foo.com\n"),
                strWhatAmI.c_str(),
